...
module vga_framebuffer(input logic        clk,
   ...
   logic [7:0] framebuffer [0:320*240-1];

   vga_counters counters(.clk50(clk), .reset(reset), .*);
   assign fb_x = hcount[10:2];
   assign fb_y = vcount[9:1];
   assign fb_addr = (fb_y << 8) + (fb_y << 6) + fb_x;
   assign pixelcolor = framebuffer[fb_addr];
integer i;
initial begin
