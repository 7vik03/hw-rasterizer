...
module avalon_interface (
    ...
    // -------- Avalon-MM slave (from HPS lightweight bridge) --------
    input  logic [6:0]      avalon_address,   // word address
    input  logic            avalon_write,
    input  logic [31:0]     avalon_writedata,
    output logic [31:0]     avalon_readdata,

    // -------- handshake to triangle_dispatcher --------
    input  logic            pop,
    ...
    localparam logic [6:0] ADDR_PACKET_HI = 7'h10;  // inclusive (17 words)
    ...
    always_comb begin
        ...
        fifo_push_data.bbox_xmin = {2'b00, stage[15][ 7: 0]};   // 10 bits
        fifo_push_data.bbox_xmax = {2'b00, stage[15][15: 8]};
        fifo_push_data.bbox_ymin = {1'b0,  stage[15][23:16]};   //  9 bits
